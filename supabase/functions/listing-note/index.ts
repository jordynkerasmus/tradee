...
// SECURITY: only an authenticated ADMIN may call this — we validate the caller's
// JWT and check their email against the admin list, so it can't be abused as an
// open email relay. Deploy with "Verify JWT" OFF (we do our own check here).
// Secrets: RESEND_API_KEY (+ Supabase auto-provides SUPABASE_URL / SUPABASE_ANON_KEY).
...
<body style="margin:0;padding:0;background:#1C1917;font-family:'Helvetica Neue',Arial,sans-serif;">
  <div style="max-width:520px;margin:0 auto;padding:2rem 1rem;">
    <div style="text-align:center;margin-bottom:1.5rem;"><div style="font-size:2rem;font-weight:900;letter-spacing:0.08em;"><span style="color:#FFFDF9;">TRA</span><span style="color:#F59E0B;">DEE</span></div></div>
    <div style="background:#292524;border:1px solid #3D3935;border-radius:12px;padding:1.5rem;">
      ...
      <div style="text-align:center;margin-top:1.5rem;"><a href="${SITE}" style="display:inline-block;background:#F59E0B;color:#1C1917;font-weight:700;padding:14px 28px;border-radius:8px;text-decoration:none;">Open my dashboard</a></div>
